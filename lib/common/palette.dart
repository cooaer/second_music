...
class PaletteGenerator extends Diagnosticable {
  ...
  static Future<PaletteGenerator> fromImage(
    ...
    assert(region == null || (region.topLeft.dx >= 0.0 && region.topLeft.dy >= 0.0),
        'Region $region is outside the image ${image.width}x${image.height}');
    assert(
        region == null ||
            (region.bottomRight.dx <= image.width && region.bottomRight.dy <= image.height),
        'Region $region is outside the image ${image.width}x${image.height}');
  ...
  void _selectSwatches() {
    final Set<PaletteTarget> allTargets =
        Set<PaletteTarget>.from((targets ?? <PaletteTarget>[]) + PaletteTarget.baseTargets);
  ...
  double _generateScore(PaletteColor paletteColor, PaletteTarget target) {
    ...
    if (target.saturationWeight > 0.0) {
      saturationScore =
          target.saturationWeight * (1.0 - (hslColor.saturation - target.targetSaturation).abs());
    }
    if (target.lightnessWeight > 0.0) {
      valueScore =
          target.lightnessWeight * (1.0 - (hslColor.lightness - target.targetLightness).abs());
    }
    if (target.populationWeight > 0.0) {
      populationScore =
          target.populationWeight * (paletteColor.population / _dominantColor.population);
    }
